...
def _get_mint_logs(chain: str, startblock: int, endblock: int, limit: int = 250) -> List[Dict[str, Any]]:
    """
    Discover token/NFT-like contracts by detecting Transfer events where:
      from = zero address

    This is a creation/mint proxy, not a raw contract creation scan.
    It is useful because real token/NFT launches usually mint initial supply/items.
    """
    data = etherscan_get(
        chain,
        {
...
def discover_base_candidates(target_date: str) -> List[Dict[str, Any]]:
    ...
    if startblock is not None and endblock is not None:
        # 1. Creation-style discovery via mint events.
    ...
    # These catch projects that are visible but may not be cleanly discovered from the mint-log scan.
